...
// FILE:	DSP2833x_DMA.c
//
// TITLE:	DSP2833x DMA Control Register Definitions and enhanced Definitions.
//
//###########################################################################
...
struct DMACH_BURST_SETS{
	Uint16 bsize;		// Transfer (bsize+1) words in a burst
	int16 srcbstep;		// Add (step) to address
	int16 desbstep;		//
};

struct DMACH_TRANSFER_SETS{
	Uint16 tsize;		// These bits specify the number of bursts to transfer
	int16 srctstep;
	int16 deststep;
};

struct DMACH_WRAP_SETS{
	Uint16 srcwsize;	// Wrap after (wsize+1) burst
	int16 srcwstep;
	Uint16 deswsize;	// Wrap after (wsize+1) burst
	int16 deswstep;
};


struct DMACH_SETS{
	struct MODE_BITS  			ModeSets;
	struct DMACH_WRAP_SETS 		WrapSets;
	struct DMACH_TRANSFER_SETS	TransferSets;
	struct DMACH_BURST_SETS  	BurstSets;
};

struct DMA_VARS {
	volatile struct  DMA_REGS  *RegsAddr;

//   *** added by QUENTIN ****
	struct DMACH_SETS Sets;
...
void DMACHxConfig(struct DMACH_SETS * dmaSets, volatile struct CH_REGS *ch);	// all in one

// specific
extern volatile Uint16 *DMACh1Dst;
extern volatile Uint16 *DMACh1Src;
extern volatile Uint16 *DMACh2Dst;
extern volatile Uint16 *DMACh2Src;
extern volatile Uint16 *DMACh3Dst;
extern volatile Uint16 *DMACh3Src;
extern volatile Uint16 *DMACh4Dst;
extern volatile Uint16 *DMACh4Src;
extern volatile Uint16 *DMACh5Dst;
extern volatile Uint16 *DMACh5Src;
extern volatile Uint16 *DMACh6Dst;
extern volatile Uint16 *DMACh6Src;
